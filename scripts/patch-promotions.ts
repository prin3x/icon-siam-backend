...
async function patchPromotions() {
  ...
  try {
    ...
    console.log(`Found ${legacyPromotions.length} promotions to update content for`)
    ...
    for (const legacyPromotion of legacyPromotions) {
      try {
        // Update existing promotion content only
        let updatedLocales = []

        if (legacyPromotion.text_th) {
          // Find existing promotion by legacyPromotion title
          ...
          if (existingPromotions.docs.length > 0) {
            await payload.update({
              collection: 'promotions',
              id: existingPromotions.docs[0].id,
              data: {
                content: decodeHtmlEntitiesNode(legacyPromotion.text_th) as any,
              },
              locale: 'th',
            })
        ...
        if (legacyPromotion.text_en) {
          const existingPromotions = await payload.find({
            collection: 'promotions',
            where: {
              title: {
                equals: legacyPromotion.title_en,
              },
            },
            locale: 'en',
            limit: 1,
          ...
          if (existingPromotions.docs.length > 0) {
            await payload.update({
              ...
              id: existingPromotions.docs[0].id,
              data: {
                content: decodeHtmlEntitiesNode(legacyPromotion.text_en) as any,
              },
              locale: 'en',
            })
        ...
        if (legacyPromotion.text_cn) {
          const existingPromotions = await payload.find({
            collection: 'promotions',
            where: {
              title: {
                equals: legacyPromotion.title_cn,
              },
            },
            locale: 'zh',
            limit: 1,
          ...
          if (existingPromotions.docs.length > 0) {
            await payload.update({
              ...
              id: existingPromotions.docs[0].id,
              data: {
                content: decodeHtmlEntitiesNode(legacyPromotion.text_cn) as any,
              },
              locale: 'zh',
            })
        ...
        if (updatedLocales.length > 0) {
          updated++
          console.log(
            `✅ Updated content for promotion ID: ${legacyPromotion.id} (locales: ${updatedLocales.join(', ')})`,
          )
        } else {
...
// Run the script
patchPromotions().catch(console.error)
