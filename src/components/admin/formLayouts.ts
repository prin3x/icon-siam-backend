...
function isLikelyAutoGeneratedField(fieldName: string): boolean {
  return (
    fieldName.toLowerCase() === 'createdat' ||
    fieldName.toLowerCase() === 'updatedat' ||
    fieldName.toLowerCase() === 'createdby' ||
    fieldName.toLowerCase() === 'updatedby' ||
  ...
  const remaining = fields
    .filter(
      (f) =>
        ...
        !isLikelySEOField(f.name) &&
        !isStatusField(f.name) &&
        !isLikelyAutoGeneratedField(f.name),
    )
    .map((f) => f.name)
