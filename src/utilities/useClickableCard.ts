...
function useClickableCard<T extends HTMLElement>({
  ...
  const handleMouseUp = useCallback(
    (e: MouseEvent) => {
      ...
      const isClick = difference <= 250
      const hasLink = link.current?.href
      const isMainButton = pressedButton.current === 0
      const hasNoActiveParent = !hasActiveParent.current
      const noCtrlKey = !e.ctrlKey

      if (!isClick || !hasLink || !isMainButton || !hasNoActiveParent || !noCtrlKey) {
        return
      }

      if (external) {
        const target = newTab ? '_blank' : '_self'
        window.open(hasLink, target)
      } else {
        router.push(hasLink, { scroll })
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
